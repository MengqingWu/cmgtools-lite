...
class BPH4lLeptonAnalyzer( Analyzer ):
    ...
    def declareHandles(self):
        super(BPH4lLeptonAnalyzer, self).declareHandles()
    ...
    def beginLoop(self, setup):
        super(BPH4lLeptonAnalyzer,self).beginLoop(setup)
    ...
    def makeLeptons(self, event):
        ...
        if self.cfg_comp.isMC:
            for i in event.selectedLeptons:
                i.hasgen=int(i.physObj.genParticle().__nonzero__())+int(i.physObj.genParticle().__nonzero__() and i.physObj.genParticle().isPromptFinalState())
    ...
    def makeAllMuons(self, event):
        ...
        for mu in allmuons:
            ...
            # follow BPH-14-006 with more info from:
            # https://twiki.cern.ch/twiki/bin/view/CMS/FourMuPassTwo#Preselection
            # FIXME: the vertex used for dxy to be verify, the track used for dxy to be verify
            mu.preMuonID = mu.physObj.isTrackerMuon() \
                           and mu.physObj.innerTrack().hitPattern().pixelLayersWithMeasurement()>0 \
    ...
    # """ mini isolation from xzz2l2v, based on b2g X->2l2j analysis
    # """
...
#A default config
setattr(BPH4lLeptonAnalyzer,"defaultConfig",cfg.Analyzer(
    verbose=False,
    class_object=BPH4lLeptonAnalyzer,
    # input collections
    muons='slimmedMuons',
