...
class BPH4lVertexAnalyzer( Analyzer ):
    ...
    def __init__(self, cfg_ana, cfg_comp, looperName):
        super(BPH4lVertexAnalyzer, self).__init__(cfg_ana, cfg_comp, looperName)
    ...
    def declareHandles(self):
        super(BPH4lVertexAnalyzer, self).declareHandles()
    ...
    def beginLoop(self, setup):
        super(BPH4lVertexAnalyzer,self).beginLoop(setup)
    ...
    def process(self,  event):
        ...
        if self.verbose:
            print 'BPH4lVertexAnalyzer: #vert = ', len(event.vertices), \
...
#           self.pileup.mindist.Fill( self.mindist(event.goodVertices) )
    ...
    def write(self, setup):
        super(BPH4lVertexAnalyzer, self).write(setup)
        if self.doHists:
            self.pileup.write()

setattr(BPH4lVertexAnalyzer,"defaultConfig",cfg.Analyzer(
    class_object=BPH4lVertexAnalyzer,
    vertexWeight = None,
    fixedWeight = 1,
    verbose = False
   )
