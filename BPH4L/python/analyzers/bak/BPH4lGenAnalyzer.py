...
class BPH4lGenAnalyzer( Analyzer ):
    ...
    def __init__(self, cfg_ana, cfg_comp, looperName ):
        super(BPH4lGenAnalyzer,self).__init__(cfg_ana,cfg_comp,looperName)
    ...
    def beginLoop(self,setup):
        ...
        self.counters.addCounter('BPH4lGenReport')
        self.count = self.counters.counter('BPH4lGenReport')
        self.count.register('BPH4L Events')
        self.count.register('BPH4l2l2j Events')
	...
	event.genTausFsr = []
        ...
        if len(event.genZBosons)>=2 and len(event.genLeptons)>=2 and len(event.genNeutrinos)>=2:
            event.genIsBPH4L = True
        else: 
            event.genIsBPH4L = False
 
        if len(event.genZBosons)>=2 and len(event.genLeptons)>=2 and len(event.genJets)>=2:
            event.genIsBPH4l2l2j = True
        else:
            event.genIsBPH4l2l2j = False
            
        if len(event.genZBosons)>=2 and len(event.genElectrons)>=1 and len(event.genMuons)>=1 and len(event.genNeutrinos)>=2:
            event.genIsBPH4lemu2nu = True
        else:
            event.genIsBPH4lemu2nu = False

        if event.genIsBPH4lemu2nu:
            self.count.inc('emu2nu Events')

        if event.genIsBPH4L:
            self.count.inc('BPH4L Events') 

        if event.genIsBPH4l2l2j:
            self.count.inc('BPH4l2l2j Events')

        
        if len(event.genZBosons)>=2 :
            genX = event.genZBosons[0]+event.genZBosons[1]
            event.genBPH4l.append(genX)
            

        if self.cfg_ana.verbose and event.genIsBPH4L:
...
setattr(BPH4lGenAnalyzer,"defaultConfig",
    cfg.Analyzer(BPH4lGenAnalyzer,
