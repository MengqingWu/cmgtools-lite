...
int main(int argc, char** argv) {
  ...
  bool doElMu_shape_corr=false;
  TFile* emusfFile; TH1D* h1_sf_Memu;
  if (doElMu_shape_corr) {
      std::cout << "[Info] : adding shape correction weight to el-mu pair event " << std::endl ;
      emusfFile = new TFile("shape_correction.root");
      h1_sf_Memu = (TH1D*)emusfFile->Get("sf_vs_Memu");
      Int_t binx = h1_sf_Memu->GetXaxis()->FindBin(190);
      std::cout<< "[test] For mz=190Gev, sf = " <<h1_sf_Memu->GetBinContent(binx)<<std::endl;
    }
  else{
    std::cout << "[Info] : a PLAIN shape correction weight (1.0) added to el-mu pair event " << std::endl ;
  }
  ...
  Float_t  elmununu_l1_l1_pt[1], elmununu_l1_l2_pt[1],
    elmununu_l1_l1_eta[1], elmununu_l1_l2_eta[1], elmununu_l1_mass[1];
  Int_t nelmununu;
  //ULong64_t evt;
  ...
  for (int i=0; i<(int)tree_tmp->GetEntries(); i++){ 
    ...
    if (nelmununu>0 ){
      ...
      if (doElMu_shape_corr){
	Int_t binx = h1_sf_Memu->GetXaxis()->FindBin(elmununu_l1_mass[0]);
	Float_t h1sf = h1_sf_Memu->GetBinContent(binx);
	Melmu_sf = (h1sf==0) ? 1.0: h1sf;
      }
      else Melmu_sf = 1.0;
    }
    else{
    ...
    tree_out->Fill();
    if (i%2000==0) std::cout<<"Event "<<i<<std::endl;
    //if( i==200) break;
  }
