...
int main(int argc, char** argv) {
  ...
  std::cout<<"debug: in tree entries = "<<tree->GetEntriesFast()<<std::endl;
  tree->SetBranchStatus("met_*",0);
  tree->SetBranchStatus("genLep*",0);
  tree->SetBranchStatus("elmununu_LV_*",0);
  tree->SetBranchStatus("llnunu_LV_*",0);

  // out_tree
  TTree* tree_out = tree->CopyTree(Cuts.c_str());
  std::cout<<"debug: out tree entries = "<<tree_out->GetEntriesFast()<<std::endl;
